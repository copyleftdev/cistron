...
reference** — both the digest (2,001 cases) and fully-justified normalization
(2,000 cases). Every crate is mutation-tested to **0 survivors**, and all parsers
are fuzzed.
...
**`identity`** is built, with two id schemes. `variant_id` is a compact internal
content-address (`cistron:va.`) whose identity law (`id equal ⟺ same denotation`)
is exhaustively tested, so equality is a hash compare. The **`vrs` module** emits
**real GA4GH VRS identifiers** (`ga4gh:VA.`, plus `ga4gh:SL.`/`ga4gh:SQ.`) that
are **byte-for-byte compatible with the vrs-python reference implementation** —
proven against the VRS 2.0 spec's worked example *and* a committed 2,001-vector
conformance corpus generated by vrs-python (`identity/tests/vrs_vectors.tsv`, run
on every `cargo test`, no Python needed). Both the digest algorithm and
**fully-justified normalization** ([`Variant::fully_justified`]) match exactly,
so `ga4gh_allele_id` reproduces vrs-python for *every* variant including
tandem-repeat indels (validated against a 2,000-vector normalization corpus).
The one exception is large repeats VRS encodes as a run-length expression, which
this crate does not yet emit.
...
- **VRS run-length expressions** — emit RLE state for large repeats, the one
  normalization case not yet covered.
