...
291,600 cases, and GA4GH VRS identifiers **byte-for-byte against the vrs-python
reference** over 2,001 cases. Every crate is mutation-tested to **0 survivors**,
and all parsers are fuzzed.
...
on every `cargo test`, no Python needed). The digest algorithm (canonical
serialization + `sha512t24u`) matches exactly; the cistron→VRS bridge uses
left-alignment, which equals VRS fully-justified normalization for every variant
outside a tandem repeat.
...
- **VRS fully-justified normalization** — so the cistron→VRS bridge matches
  vrs-python end-to-end for tandem-repeat indels too (the digest already does).
